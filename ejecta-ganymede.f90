    ...
    implicit none
     ...
     real(8) :: gm,distance,w, radius,lunch,rotation,c_radius
     ...
     integer*1 :: ini(54), col(1000,-90:90,3)
     real(8) :: maxdensity,mindensity
     integer :: Target, bina
     real(8) :: colat, coarea,hill_radius
   ...
   !!!!!!!!parameter setting
   volaspect= 45. !azimuth of uprange, anti-crockwise from the east
   ...
   c_radius = 800.*1000.   !transient crater radius in meter
 ...
 open(14,file="T1E.bmp", access='direct',recl=1)
    ...
    dt = 1.d0   !time step
    radius = 2634.*1000.
    c_lat = dble(CENTERLATITUDE) *pi/180.d0 
    c_lon = dble(CENTERLONGITUDE) *pi/180.d0 
    c_lon = c_lon -pi
   ...
   coarea  = 2.d0*pi*radius*radius*(1.d0-dcos(colat))
    k =0
    k2=0
    rotation = dble(7.154 *24.) ! rotation period
    w = 2.*pi/(rotation*3600.) !rad/s 
    gm =1.4819d23*6.67408d-11  
    g = gm/(radius*radius)
  hill_radius = 1070400.*1000.*(1.4819d23/3./1.898d27)**(1./3.)
 ...
 do loop2 = 1, num2 
  ...
  do loop = 1, num-1
    ...
    !initial position and velosity adjustment 
    cx = pp(1)
    cy = pp(2)
    cz = pp(3)
    ...
    pv(2) = mcy
    pv(3) = mcz
   !initial position and velosity adjustment end
  ...
  !Adams-Bashforth Method
      ...
      do j = 1, int(1000000./dt)
        ...
        !centrifugal force
         ...
         dx(3) = pv(3)*dt
         do i = 1, 3
         pv(i)=pv(i)+1.5*dp(i)-0.5*dp_prev(i)  !velocity
         pp(i)=pp(i)+1.5*dx(i)-0.5*dx_prev(i)  !position
         dp_prev(i) = dp(i)
         dx_prev(i) = dx(i)
         enddo
  ...
  distance = dacos(dsin(rtem1)*dsin(clat)+  &
  dcos(rtem1)*dcos(clat)*dcos(rtem2-clon))
    if ( distance .le.  colat) then
